...
Future<void> processCsvFile(String inputPath, String outputPath) async {
  ...
  for (int i = startIndex; i < lines.length; i++) {
    ...
    if (originalWord == "다지" || originalWord.contains("다지")) {
      print(
          "디버깅: 발견된 단어: '$originalWord', 열 개수: ${columns.length}, 두 번째 열: ${columns.length > 1 ? columns[1] : '없음'}");
    }
    ...
    if (originalWord.contains('-')) {
      ...
      if (isValidKoreanWord(cleanedWord) &&
          !uniqueWords.contains(cleanedWord)) {
...
Future<void> splitByConsonant(List<String> words, String basePath) async {
  ...
  for (final word in words) {
    ...
    if (!consonantMap.containsKey(consonant)) {
      print('초성 기타에 대한 인덱스 정보 없음: $consonant (단어: $word)');
      consonantMap['기타'] = consonantMap['기타'] ?? [];
      consonantMap['기타']!.add(word);
    } else {
  ...
  for (final consonant in consonants) {
    ...
    if (wordList.isEmpty) {
      print('$consonant 초성에 해당하는 단어가 없습니다.');
      indexMap[consonant] = 0;
      continue;
    }
...
String getInitialConsonant(String char) {
  ...
  if (code < 0xAC00 || code > 0xD7A3) {
    print('한글 범위 밖의 문자 발견: $char (코드: ${code.toRadixString(16)}), 기타로 분류됨');
    return '기타';
  }
  ...
  if (index >= 0 && index < initialConsonants.length) {
    ...
    print('초성 인덱스 범위 초과: $index (문자: $char), 기타로 분류됨');
    return '기타';
  }
}
